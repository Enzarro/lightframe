...
class sys_tree_model {
    ...
    function set($data_crud) {
        ...
        if (isset($data_crud["permisos_obj"])) {
            ...
            $data_crud["permisos_obj"] = array_filter($data_crud["permisos_obj"], function($permiso) {
    ...
    function get($id) {
        ...
        $res = $_DB->queryToArray("SELECT parent_id, texto, icono, funcion, orden, grid_id, permisos_obj FROM {$this->table} WHERE (eliminado != 1 OR eliminado IS NULL) AND {$this->primaryKey} = {$id}");
        $res = (object)$res[0];
