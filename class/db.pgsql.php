...
class database {
    ...
    function db_connect() {
        ob_start();
        $conn = pg_connect("host=$this->host port=$this->port dbname=$this->database user=$this->user password=$this->password connect_timeout=0");
        // $conn = pg_connect("host=$this->host port=$this->port user=$this->user password=$this->password");
        if ($conn) {
            $this->conn = $conn;
            // $this->query("SET search_path TO datcapital;");
            return $this->conn;
        }
        else {
